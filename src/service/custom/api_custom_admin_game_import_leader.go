...
// /api/custom/admin/game/import/leader Service 从文件导入领导信息，文件格式参考领导模板
func ApiCustomAdminGameImportLeaderService(
    user *master_types.User,
    GameId master_types.PrimaryId,
    LeaderFileId master_types.PrimaryId,
) (*master_types.MasterResponse) {
    var apiCustomAdminGameImportLeaderResponse struct {
...
/* @MT-TPL-SERVICE-END */
	...
	for i, row := range rows {
		...
		if phone_obj == nil || err != nil {
			...
			user_model := &master_types.User{
				Username: "领导_" + phone[0:phone_max_length],
				Parent:   user.Id,
			}
		...
		gamer_model := &master_types.Gamer{
			Owner:    uid,
			GameId:   GameId,
			Name:     "领导_" + strconv.Itoa(num.Random(1000, 9999)),
			Phone:    phone,
			CreateAt: time.Now().Unix(),
			Score:    0,
		}
		gamer_model.SetCustomer()
	...
	apiCustomAdminGameImportLeaderResponse.FileId = id

	/* @MT-TPL-SERVICE-RESP-START */

    return master_types.SuccessResponse(apiCustomAdminGameImportLeaderResponse)
}
