...
// /api/custom/defender/statistics Service 蓝队获取统计信息
func ApiCustomDefenderStatisticsService(
    user *master_types.User,
    GameId master_types.PrimaryId,
) (*master_types.MasterResponse) {
    var apiCustomDefenderStatisticsResponse struct {
...
/* @MT-TPL-SERVICE-END */
	...
	// get all reports
	reports, err := model.ModelGetAll[types.Report](
		model.NewMongoFilter(
			model.MongoKeyFilter("game_id", GameId),
			model.MongoKeyFilter("report_type", types.REPORT_TYPE_ATTACK),
		),
		&model.MongoOptions{
	...
	attack_types := types.GetReportAttackTypes() // gimmack
	vuln_types := types.GetReportVulnTypes()     // vuln_type
	vuln_levels := types.GetReportLevels()       // vuln_level
	...
	var defender_ids []types.PrimaryId
	...
	for reports_index := range reports {
		...
		// get report state
		switch report.State {
		case types.REPORT_STATE_ACCEPTED:
			statistics.Reports.Accepted++
		case types.REPORT_STATE_REJECTED:
			statistics.Reports.Rejected++
		case types.REPORT_STATE_UNVERIFIED:
			statistics.Reports.Unverified++
		}

		// get isolation break
		if report.IsolationBreak == 1 && report.State == types.REPORT_STATE_ACCEPTED {
			statistics.Reports.IsolationBreak++
		}
	}

	// get all defenders
	defenders, err := model.ModelGetAll[types.Defender](
		model.NewMongoFilter(
			model.MongoKeyFilter("game_id", GameId),
	...
	var defender_id_industry_map = make(map[types.PrimaryId]string)
	...
	// get red team
	red_team, err := model.ModelGetAll[types.RedTeam](
		model.NewMongoFilter(
			model.MongoKeyFilter("game_id", GameId),
		),
		&model.MongoOptions{
	...
	var defender_map = make(map[types.PrimaryId]*struct {
		Defender types.Defender `json:"defender" bson:"defender"`
		Count    int64          `json:"count" bson:"count"`
	})
	for defenders_index := range defenders {
	...
	for _, report := range reports {
		if report.State == types.REPORT_STATE_ACCEPTED {
	...
	// get boardcast
	boardcasts, err := model.ModelGetAll[types.Boardcast](
		model.NewMongoFilter(
			model.MongoKeyFilter("game_id", GameId),
		),
		&model.MongoOptions{
	...
	apiCustomDefenderStatisticsResponse.Statistics = statistics

	/* @MT-TPL-SERVICE-RESP-START */

    return master_types.SuccessResponse(apiCustomDefenderStatisticsResponse)
}
