...
/* @MT-TPL-SERVICE-END */
	...
	for i, row := range rows {
		...
		if phone_obj == nil || err != nil {
			...
			user_model := &master_types.User{
				Username: "裁判_" + phone[0:phone_max_length],
				Parent:   user.Id,
			}
		...
		gamer_model := &master_types.Gamer{
			Owner:    uid,
			GameId:   GameId,
			Name:     "裁判_" + strconv.Itoa(num.Random(1000, 9999)),
			Phone:    phone,
			CreateAt: time.Now().Unix(),
			Score:    0,
		}
		gamer_model.SetJudgement()
	...
	apiCustomAdminGameImportJudgeResponse.FileId = id
