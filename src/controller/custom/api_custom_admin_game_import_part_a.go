...
type ApiCustomAdminGameImportPartAController struct {
    beego.Controller
}

func (c *ApiCustomAdminGameImportPartAController) Post() {
    ...
    request_params_part_a_file_id, err := primitive.ObjectIDFromHex(c.GetString("part_a_file_id"))
    if err != nil {
        c.Ctx.Output.JSON(master_types.ErrorResponse(-400, err.Error()), true, false)
        return
    }
    ...
    response := custom_service.ApiCustomAdminGameImportPartAService(
        user,
        request_params_game_id,
        request_params_part_a_file_id,
    )
