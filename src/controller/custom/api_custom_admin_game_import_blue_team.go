...
type ApiCustomAdminGameImportBlueTeamController struct {
    beego.Controller
}

func (c *ApiCustomAdminGameImportBlueTeamController) Post() {
    ...
    request_params_blue_team_file_id, err := primitive.ObjectIDFromHex(c.GetString("blue_team_file_id"))
    if err != nil {
        c.Ctx.Output.JSON(master_types.ErrorResponse(-400, err.Error()), true, false)
        return
    }
    ...
    response := custom_service.ApiCustomAdminGameImportBlueTeamService(
        user,
        request_params_game_id,
        request_params_blue_team_file_id,
    )
