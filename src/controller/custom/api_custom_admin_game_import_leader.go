...
type ApiCustomAdminGameImportLeaderController struct {
    beego.Controller
}

func (c *ApiCustomAdminGameImportLeaderController) Post() {
    ...
    request_params_leader_file_id, err := primitive.ObjectIDFromHex(c.GetString("leader_file_id"))
    if err != nil {
        c.Ctx.Output.JSON(master_types.ErrorResponse(-400, err.Error()), true, false)
        return
    }
    ...
    response := custom_service.ApiCustomAdminGameImportLeaderService(
        user,
        request_params_game_id,
        request_params_leader_file_id,
    )
